...
| D-8 | Orchestration hub | **`ORCHESTRATION.md` (repo root) is the canonical hub** + entry point across all Claude surfaces; `THREAD-SEED.md` is the portable copy-paste boot block for new threads (mirror to phone/iCloud). Anti-spiral rules: one workstream/thread, repo = source of truth, new-version-never-overwrite, agents for fan-out, ship-at-8. Hub carries the surface map, plugins/setup guidance, design→code + card-naming workflows, connector inventory, the live workstream board, and the PR triage. Supersedes scattered `_HANDOFF/`/`PROMPTS/` docs in the Dropbox "ABOUT BILL" folder as the source of truth. | 2026-06-23 |
